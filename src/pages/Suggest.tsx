...
export default function Suggest() {
  ...
  return (
    <div className="mx-auto max-w-6xl overflow-x-clip px-6 pb-24 pt-32 md:pt-40">
      ...
      {/* Wishlist */}
      <section id="wishlist" className="mt-20 min-w-0 scroll-mt-28">
        <Reveal>
        ...
        {loadError ? (
          <Reveal delay={0.05}>
            <div className="mt-10 rounded-3xl border border-amber-500/30 bg-amber-50 px-6 py-6">
              <p className="font-display font-bold text-amber-900">
                আগের সাজেশনগুলো লোড হচ্ছে না
              </p>
              <p className="mt-1.5 text-sm leading-relaxed text-amber-800/90">
                Firestore Rules-এ <code className="rounded bg-white/70 px-1.5 py-0.5">suggestions</code>{' '}
                কালেকশনের জন্য{' '}
                <code className="rounded bg-white/70 px-1.5 py-0.5">allow read: if true;</code>{' '}
                দিলেই এই পাবলিক wishlist দেখা যাবে। নতুন সাজেশন জমা হওয়া ঠিকঠাক কাজ করছে।
              </p>
            </div>
          </Reveal>
        ) : loading ? (
          <div className="mt-10 grid min-w-0 gap-5 md:grid-cols-2 [&>*]:min-w-0">
          ...
          <div className="mt-10 grid min-w-0 gap-5 md:grid-cols-2 [&>*]:min-w-0">
            {suggestions.map((idea, i) => (
              <Reveal key={idea.id} delay={Math.min(i, 5) * 0.05}>
...
/**
 * Wishlist card. Long descriptions are clamped to 4 lines so the grid
 * stays evenly sized; a Read more / Show less toggle expands in place.
 * Words also break safely so URLs never overflow the card.
 */
    ...
    <article className="flex h-full min-w-0 max-w-full flex-col overflow-hidden rounded-3xl border border-ink/10 bg-white p-7 transition-shadow duration-300 hover:shadow-xl hover:shadow-ink/5">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-2 break-words font-display text-xl font-bold tracking-tight">
            {idea.title}
          </h3>
        </div>
        <span
      ...
      <div className="mt-3 flex-1">
        <p
          className={`break-words text-base leading-relaxed text-smoke ${
